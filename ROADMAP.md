...
| 里程碑 | 版本 | BUILD 范围 | 状态 |
|--------|------|-----------|------|
| M1 | **0.2.0-M1** | Phase 1-2 | ✅ 已完成 |
| M2 | **0.2.0-M2** | Phase 3-9 | ✅ 已完成 |
| M3 | **0.2.0-M3** | Phase 7-8（服务+事件）| ✅ 已完成 |
| M4 | **0.2.0-M4** | Phase 9（网络层）| ✅ 已完成 |
| M5 | **0.2.0-M5** | Phase 10-12（审计+Daemon+自演化）| ✅ 已完成（"不死鸟"）|
| M6 | **0.2.0-M6** | Phase 13-16（MC深度集成）| ✅ 已完成 |
| M7 | **0.2.0-M7** | Phase 17（Mixin适配器）| ✅ 已完成 |
| M8 | **0.2.0-M8** | Phase 18（物品注册接管）| 🚧 规划中 |
| M9 | **0.2.0-M9** | P0 决策 + API 冻结 | 📋 待启动 |
| M10 | **0.2.0** | 开发者体验 → 正式发布 | 📋 待启动 |

> **说明：** 所有里程碑均归属于 v0.2.0 系列主线，以 `-Mx` 后缀区分各个里程碑版本。最终正式版为 `0.2.0`（即 M10 达成时）。
