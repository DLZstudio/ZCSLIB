...
import zcslib.mixin.MixinAdapter;
...
public class ZCSLIB {
    ...
    /** Plugin search root: {@code config/DLZstudio/ZCSLIB/plugins/} */
    private static final Path PLUGINS_DIR =
            FMLPaths.GAMEDIR.get().resolve("config").resolve("DLZstudio").resolve("ZCSLIB").resolve("plugins");

    public ZCSLIB(IEventBus modBus, ModContainer container) {
        splash();
        ZCSLIBCommon.bootstrap();

        MixinAdapter.bootstrap(PLUGINS_DIR);
